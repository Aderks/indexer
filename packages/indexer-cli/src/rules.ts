...
const INDEXING_RULE_PARSERS: Record<keyof IndexingRuleAttributes, (x: never) => any> = {
  ...
  allocationMaxLifetime: nullPassThrough(parseInt),
...
> = {
  ...
  allocationMaxLifetime: nullPassThrough((x: number) => x.toString()),
...
> = {
  ...
  allocationMaxLifetime: nullPassThrough((x: string) => parseInt(x)),
...
> = {
  ...
  allocationMaxLifetime: nullPassThrough((x: number) => x),
